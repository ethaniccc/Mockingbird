...
class UserDebugCommand extends Command implements PluginOwned{
...
<<<<<<< HEAD
					...
					$p = $sender->getServer()->getPlayerExact($selectedCheat);
					if($p !== null && $u !== null && ($tu = UserManager::getInstance()->get($p)) !== null){
						$tu->mouseRecorder = new MouseRecorder($u, $this->getOwningPlugin()->getDataFolder() . 'mouse_recordings/' . $p->getName() . '.png', (int) $args[2] ?? 25, $args[3] ?? 1);
						$tu->mouseRecorder->start();
						$u->sendMessage('You have started a mouse recording for ' . $p->getName() . ' lasting ' . ($args[2] ?? 25) . ' seconds');
					}elseif($p === null){
						$sender->sendMessage($this->getOwningPlugin()->getPrefix() . TextFormat::RED . ' Could not find the player ' . $selectedCheat);
					}
					return;
				...
				if($user === null){
					$sender->sendMessage($this->getOwningPlugin()->getPrefix() . TextFormat::RED . " Could not find the user $selectedUser");
				}else{
