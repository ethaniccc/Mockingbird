...
abstract class Detection{
	...
	protected int $vlSecondCount = 2;
	protected float $lowMax;
	protected float $mediumMax;
	...
	public abstract function handleSend(DataPacket $packet, User $user) : void;
	...
	protected function fail(User $user, ?string $debugData = null, ?string $detailData = null) : void{
		...
		if($debugData !== null){
			$nameKey = strtolower($this->name ?? '');
			if(!isset($user->debugCache[$nameKey])){
				$user->debugCache[$nameKey] = '';
			}
			$user->debugCache[$nameKey] .= $debugData . PHP_EOL;
